...
#### Класс ClientData
Класс отвечает за хранение и логику работы с данными пользователя, оформляющего заказ.\
Конструктор класса принимает инстант брокера событий.\
В полях класса хранятся следующие данные:
- `_paymentMethod: string` - способ оплаты заказа;
- `_address: string` - адрес доставки;
- `_email: string` - контактный адрес эл.почты пользователя;
- `_phone: string` - контактный телефон;
- `events: IEvents` - экземпляр класса `EventEmitter` для инициации событий при изменении данных.

Также класс предоставляет набор методов для взаимодействия с перечисленными данными:
- `getInfo(): IClientInfo` - получает введенные пользователем данные;
- `setInfo(clientData: IClientInfo): void` - отправляет пользовательские данные (например, этот метод может понадобиться при отправке *post*-запроса на сервер).
